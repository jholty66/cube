...
The best way I found to figure out how the algorithm would to create a
net for a given shape was to methodically draw a net step by step on
paper. Then write the procudure using the steps and then draw the net
that was outputted.
...
#+INCLUDE: "../python/cube.py" src python
** =/www:=
